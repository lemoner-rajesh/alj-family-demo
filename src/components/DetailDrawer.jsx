...
export default function DetailDrawer({ person, generationLabel, relations, onClose, onJump }) {
  ...
  return (
    <aside className="drawer" role="dialog" aria-label={`${person.name} details`}>
      ...
      {!person.unverified && person.note && (
        <div className="drawer__note drawer__note--context">
          <p>{person.note}</p>
        </div>
      )}
